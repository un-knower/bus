...
/**
  ...
  */
object BusArrivalAppTest2 {
  def main(args: Array[String]): Unit = {
    val inputPath = "E:/busdata/STRING_20171202"
    val outputPath = "E:/data/busArrival/2017-12-02"
    val url = "jdbc:mysql://172.16.3.200:3306/xbus_v2?user=xbpeng&password=xbpeng"
    val isSave = 1

    val spark = SparkSession.builder().master("local[*]").appName("BusArrivalApp2").getOrCreate()
    ...
    val df = group.mapGroups((key, it) => {
      ...
      BusArrivalMySQL200(startStation.tripId, currentDate, currentDate, carId, startStation.direct, endStation.stationId, endStation.stationIndex.toInt, new Timestamp(sdf.parse(endStation.arrivalTime).getTime)
        ...
        , startStation.stationId, startStation.stationIndex, new Timestamp(sdf.parse(startStation.arrivalTime).getTime), trip_timecost.toInt, s"$length/${startStation.total}", startStation.trip_mile.toString, startStation.tripId)
    }).toDF()
